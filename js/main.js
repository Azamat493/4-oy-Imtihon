...
const favoriteFilledSvg = `<svg width="20" height="20" viewBox="0 0 20 20" fill="#ef4444" xmlns="http://www.w3.org/2000/svg">
...
const favoriteEmptySvg = `<svg width="20" height="20" viewBox="0 0 20 20" fill="#888888" xmlns="http://www.w3.org/2000/svg">
<path opacity="0.4" d="M1.85938 11.7344L8.91797 18.3242C9.21094 18.5977 9.59766 18.75 10 18.75C10.4023 18.75 10.7891 18.5977 11.082 18.3242L18.1406 11.7344C19.3281 10.6289 20 9.07815 20 7.45706V7.23049C20 4.50003 18.0273 2.1719 15.3359 1.72268C13.5547 1.42581 11.7422 2.00784 10.4688 3.28128L10 3.75003L9.53125 3.28128C8.25781 2.00784 6.44531 1.42581 4.66406 1.72268C1.97266 2.1719 0 4.50003 0 7.23049V7.45706C0 9.07815 0.671875 10.6289 1.85938 11.7344Z"/></svg>`;
...
function createProductCard(product) {
  ...
  card.className =
    "rounded-lg shadow-sm hover:shadow-lg transition-transform duration-300 flex flex-col relative";
  ...
  const stockHtml =
    product.inStock === false
      ? `<span class="bg-[#ffe2e5] font-bold text-sm text-center text-[#f64e60] font-family w-[120px] h-[18px] rounded-[15px]">Нет в наличии</span>`
      : "";
  ...
  card.innerHTML = `
    <div class="overflow-hidden relative">
      ...
      <button class="absolute top-2 right-2 cursor-pointer p-1" data-action="favorite">${
        isFav ? favoriteFilledSvg : favoriteEmptySvg
      }</button>
    </div>
    <div class="p-2 flex-1 flex flex-col justify-between">
      <h3 class="font-normal text-[15px] leading-[128%] text-black font-family mb-1">${desc}</h3>
      ${ratingHtml}
      ${stockHtml}
      <div class="flex justify-between items-center">
        <div class="flex flex-col items-start">
          ...
          ${
            product.oldPrice
              ? `<span class="font-normal text-[13px] line-through text-gray-400 font-family">${product.oldPrice.toLocaleString()} сум</span>`
              : ""
          }
        </div>
  ...
  if (isInCart) {
    ...
    circle.className = `
      ...
      w-6 h-6 rounded-full bg-green-500 text-white 
